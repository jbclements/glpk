...
;; for now, let's not worry about provides:
(provide (all-defined-out))


(define-ffi-definer define-glpk (ffi-lib "libglpk"))

     
(define _PROB-pointer (_cpointer 'PROB))

;; is this a _PROB-pointer?
(define (glpk-problem? cptr)
  (and (cpointer? cptr)
       (cpointer-has-tag? cptr 'PROB)))

;; I pulled this out of the source code. It should really be in
;; the header file somewhere!

;; maximal # of rows in the problem object
(define M_MAX 100000000)
;;;#define M_MAX 100000000 /* = 100*10^6 */
;;;/* maximal number of rows in the problem object */

;; maximal # of columns in the problem object
(define N_MAX 100000000)
;;;#define N_MAX 100000000 /* = 100*10^6 */
;;;/* maximal number of columns in the problem object */

;; maximal # of constraint coefficients in the problem object
(define NNZ_MAX 500000000)
...
(define _obj_dir (_enum '(GLP_MIN = 1
                          GLP_MAX)
                        _int))
...
(define _bounds-type (_enum '(GLP_FR = 1
                              GLP_LO
                              GLP_UP
                              GLP_DB
                              GLP_FX)))
...
(define-cstruct _glp_smcp ([msg_lev (_enum '(GLP_MSG_OFF
                                             GLP_MSG_ERR
                                             GLP_MSG_ON
                                             GLP_MSG_ALL
                                             GLP_MSG_DEBUG)
                                           _int)]
                           [meth (_enum '(GLP_PRIMAL = 1
                                                     GLP_DUALP
                                                     GLP_DUAL)
                                        _int)]
                           [pricing (_enum '(GLP_PT_STD = #x11
                                             GLP_PT_PSE = #x22)
                                           _int)]
                           [r_test (_enum '(GLP_RT_STD = #x11
                                            GLP_RT_HAR = #x22
                                            GLP_RT_FLIP = #x33)
                                          _int)]
                           [tol_bnd _double]
...
;;;
(define-glpk glp_set_prob_name (_fun _PROB-pointer _string -> _void))
...
(define-glpk glp_set_obj_dir (_fun _PROB-pointer _obj_dir -> _void))
;;;
;;;int glp_add_rows(glp_prob *P, int nrs);
;;;/* add new rows to problem object */

(define-glpk glp_add_rows (_fun _PROB-pointer _int -> _int))
;;;
;;;int glp_add_cols(glp_prob *P, int ncs);
;;;/* add new columns to problem object */
;; returns index of first new column
(define-glpk glp_add_cols (_fun _PROB-pointer _int -> _int))
;;;
...
(define-glpk glp_set_row_bnds (_fun _PROB-pointer _int
                                    _bounds-type _double _double
                                    -> _void))
;;;
...
(define-glpk glp_set_col_bnds (_fun _PROB-pointer _int
                                    _bounds-type _double _double
                                    -> _void))
;;;
...
(define-glpk glp_set_obj_coef (_fun _PROB-pointer _int _double ->
                                    _void))
;;;
...
(define-glpk glp_load_matrix (_fun _PROB-pointer _int
                                   _pointer #;(_array _int 1)
                                   _pointer #;(_array _int 1)
                                   _pointer #;(_array _double 1)
                                   -> _void))
;;;
...
;; appears earlier (for use in deallocator clause...)
...
(define-glpk glp_simplex (_fun _PROB-pointer
                               _glp_smcp-pointer/null
                               -> _resultcode))
;;;
...
(define-glpk glp_get_obj_val (_fun _PROB-pointer -> _double))
;;;
...
(define-glpk glp_get_col_prim (_fun _PROB-pointer _int -> _double))
;;;
...
(define-glpk glp_term_out (_fun _glp_on_off -> _glp_on_off))
;;;
